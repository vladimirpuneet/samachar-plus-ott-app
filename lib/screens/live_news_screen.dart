...
class _LiveNewsScreenState extends State<LiveNewsScreen> {
  ...
  void _loadChannels() {
    ...
    try {
      ...
      // Filter national channels
      final nationalChannels = channels.where((c) => c.category == 'NATIONAL').toList();
      
      // Group by subCategory
      final Map<String, List<LiveChannel>> grouped = {};
      for (final channel in nationalChannels) {
      ...
      // Order by priority: HINDI, ENGLISH, BUSINESS
      final subCategoryOrder = ['HINDI', 'ENGLISH', 'BUSINESS'];
      final Map<String, List<LiveChannel>> ordered = {};
      
      for (final subCategory in subCategoryOrder) {
        if (grouped.containsKey(subCategory)) {
          ordered[subCategory] = grouped[subCategory]!;
        }
      }
      
      setState(() {
        _categorizedChannels = ordered;
        _isLoading = false;
      });

    } catch (err) {
      setState(() {
        _error = "Failed to load live channels.";
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildCategorySection(String subCategory, List<LiveChannel> channels) {
    return Padding(
